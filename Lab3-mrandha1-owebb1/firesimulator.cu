...
// TODO: define and use more constants for your solution

/********** type defs **************/
//  define any struct types here (you may want to pass a struct
//  of fire simulator problem state between method functionss and helper
//  functions.  If so, define that struct type here.
//  However: you may not need a struct type and may be able accomplish this
//  completely by adding individual data members to the
//  fireSimulatorKernel class
...
// TODO: define more function prototypes here: for good modular design!

/********** CUDA Kernel prototypes **************/
// TODO: this is a kernel for initializing state for each thread's
...
// Feel free to modify the parameter list if you'd like:
__global__ void  init_rand(curandState *rand_state, int n_cols);



// TODO: you will define two cuda kernels that perform the main
//       firesimultation computation, that you will call in the
//       fireSimulatorKernel update method:

// TODO: this cuda kernel performs the main fire simulation step
//       figure out the parameters you want to pass it
//       (any pointers params should be to CUDA memory not CPU memory)
// TODO: update the prototype and function definition with params you add

__global__ void  fire_sim_kernel(curandState* state,int* burned_keeper,
            int* data, int n_cols, int n_rows,float prob, int step);

// TODO: this is the animation image update kernel
//       it should take the color3 image buffer and the fire simulation
//       state and color each pixel value based the state
// TODO: update the prototype and function definition with params you add
__global__ void  fire_to_color(color3 *img_buf, int* burned_keeper,
                                int* data, int n_cols) ;

// TODO: add __device__ functions that can be called from __global__
...
fireSimulatorKernel::~fireSimulatorKernel() {
  ...
  printf("I have deleted the kernel and everything\n");
...
void process_args(int ac, char *av[], int* iters, int* step, float* prob, int* lighting_strike, int* numb_lakes, int** lakes, char* filename){
  ...
  if(f==0){
    ...
    // Set 3 default lakes when run via command line
    int num_buckets=*numb_lakes*4;
    *lakes=(int*) malloc(sizeof(int)*(num_buckets));
    (*lakes)[0] = N/4;
    (*lakes)[1] = N/4;
...
/**************************************************************/
// TODO: implements one step of fire simulation and its animation
//       should call two cuda kernels: one to do a simulation step
//       on the world, another to animate the current state of the
//       world (update image_buff)
...
// TODO: figure out what parameters you want to pass this (you will
//       want to pass it the curandState * state for each thread
//       to generate random values, plus many other params)
//       any pointer params must be CUDA memory not CPU
// Function: For each cell, determine its next state for the iteration
